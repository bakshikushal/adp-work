...
export default class MultiSelectPicklist extends LightningElement {
    ...
    @track selectionlimit = 2;
    @track showselectall = false;
    @track errors;
    @track selectedProfileNames;
    ...
    get filteredResults() {
        ...
        if (this.valuesVal != null && this.valuesVal.length != 0) {
            if (this.valuesVal) {
                ...
                // console.log('Selected Profiles Array ', selectedProfileNames);
                // this.dispatchEvent(new CustomEvent('select', {
                //     detail: {
                //         'selectedStates' : JSON.stringify(selectedProfileNames)
                //     }
                // }))
                this.dispatchStateSelectedEvent();
                return this.valuesVal.map(profile => {

                    //below logic is used to show check mark (✓) in dropdown checklist
                    const isChecked = this.selectedProfileNames.includes(profile.Name);
                    return {
                        ...profile,
                        isChecked
                    };
    ...
    handleSelection(event) {
        ...
        if (this.selectedItems.length < this.selectionlimit) {
            ...
            if (isChecked) {
                console.log('in the handle selection for error message : ',this.selectedItems.length);
                this.showDropdown = false;
                this.errormessage();
            }
            else {
    ...
    handleRemove(event) {
        console.log('IN REMOVE ',event.target.name);
        const valueRemoved = event.target.name;
        this.selectedItems = this.selectedItems.filter(profile => profile.Id !== valueRemoved);
        console.log('AFTER REMOVE : ',JSON.stringify(this.selectedItems));
    ...
    dispatchStateSelectedEvent(){
        this.selectedProfileNames = this.selectedItems.map(profile => profile.Name);
        console.log('selectedProfileNames ', JSON.stringify(this.selectedProfileNames));
        console.log('Selected Profiles Array ', this.selectedProfileNames);
        this.dispatchEvent(new CustomEvent('select', {
            detail: {
                'selectedStates' : JSON.stringify(this.selectedProfileNames)
            }
        }))
    }
    ...
    errormessage() {
        this.errors = {
            "Search States": "Maximum of " + this.selectionlimit + " states can be selected",
        };
        this.template.querySelectorAll("lightning-input").forEach(item => {
            let label = item.label;
            if (label == 'Search States') {
